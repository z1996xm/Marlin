...
class TMCMarlin<TMC2240Stepper, AXIS_LETTER, DRIVER_ID, AXIS_ID> : public TMC2240Stepper, public TMCStorage<AXIS_LETTER, DRIVER_ID> {
  public:
		TMCMarlin(const uint16_t cs_pin, const uint8_t axis_chain_index) :TMC2240Stepper(cs_pin, axis_chain_index){}
		TMCMarlin(const uint16_t CS, const uint16_t pinMOSI, const uint16_t pinMISO, const uint16_t pinSCK, const uint8_t axis_chain_index) :TMC2240Stepper(CS, pinMOSI, pinMISO, pinSCK ,axis_chain_index ){}
    //uint8_t get_address() { return slave_address; }
    ...
    static constexpr uint8_t sgt_min = -64,
                             sgt_max = 63;
};
